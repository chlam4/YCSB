 ...
 * 
 * CREATE TABLE IF NOT EXISTS mydb( resource text, metric text, date text,
 * event_time timestamp, value float, PRIMARY KEY((resource, metric, date),
 * event_time) );
 * 
 * ALTER TABLE mydb WITH compaction = { 'class': 'DateTieredCompactionStrategy',
 * 'timestamp_resolution':'MICROSECONDS', 'base_time_seconds':'3600',
 * 'max_sstable_age_days':'365' };
 * 
 * Or with a simplified schema where the metric id is an integer (see
 * intMetricIds flag) :
 * 
 * CREATE KEYSPACE ycsb WITH REPLICATION = { 'class' : 'SimpleStrategy',
 * 'replication_factor': 1 };
 * 
 * CREATE TABLE IF NOT EXISTS mydb10( metric bigint, event_time int, value
 * float, PRIMARY KEY((metric), event_time) );
 *
 * ALTER TABLE mydb10 WITH compaction = { 'class':
 * 'DateTieredCompactionStrategy', 'timestamp_resolution':'MICROSECONDS',
...
public class CassandraCqlTsClient extends DB {
  ...
  public void init() throws DBException {
    ...
    synchronized (INIT_COUNT) {
      ...
      try {
        ...
        debug = Boolean.parseBoolean(getProperties().getProperty(
            "cassandra.debug", "false"));
        intMetricIds = Boolean.parseBoolean(getProperties().getProperty(
            "cassandra.intMetricIds", "false"));
        ...
        fetchSize = Integer.parseInt(getProperties().getProperty(
            "cassandra.fetchsize", "500"));
        ...
        int threadcount = Integer.parseInt(getProperties().getProperty(
            "threadcount", "1"));
  ...
  public void cleanup() throws DBException {
    ...
    synchronized (INIT_COUNT) {
      ...
      if (curInitCount < 0) {
        ...
        throw new DBException(String.format("INIT_COUNT is negative: %d",
            curInitCount));
      }
    }
  }
  ...
  @Override
  public Status scanDatapoints(final String table, final String key,
    ...
    try {
      ...
      if (intMetricIds) {
        ...
        stmt = selectBuilder
            .from(table)
            .where(QueryBuilder.eq("metric", Long.parseLong(key.substring(11))))
        ...
        stmt = selectBuilder.from(table)
            .where(QueryBuilder.eq("resource", key))
      ...
      while (!future.isDone()) {
        // Need something to satisfy the check style
        Thread.sleep(0, 1);
      }

      ResultSet rs = future.get(); // Shall we parse the query results?
      int rowCount = 0;
